...
public class ObserverAngry : MyMonoBehavior
{
    private static ObserverAngry instance;
    public static ObserverAngry Instance => instance;

    private List<IEmojiAngry> observers = new List<IEmojiAngry>();
    ...
    public void AddObserver(IEmojiAngry observerShot)
    {
        if (observers.Contains(observerShot) == true) return;
        observers.Add(observerShot);
    }

    public void RemoveObserver(IEmojiAngry observerShot)
    {
        tagetPos = Vector3.zero;
    ...
    public virtual void OnActiveInterface()
    {
        ...
        foreach (IEmojiAngry observer in observers)
        {
