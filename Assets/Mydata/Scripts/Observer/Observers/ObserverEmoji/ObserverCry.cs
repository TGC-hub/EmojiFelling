...
public class ObserverCry : MyMonoBehavior
{
    private static ObserverCry instance;
    public static ObserverCry Instance => instance;

    private List<IEmojiCry> observers = new List<IEmojiCry>();
    ...
    public void AddObserver(IEmojiCry observerShot)
    {
        if(observers.Contains(observerShot) == true) return;
        observers.Add(observerShot);
    }

    public void RemoveObserver(IEmojiCry observerShot)
    {
        tagetPos = Vector3.zero;
    ...
    public virtual void OnActiveInterface()
    {
        ...
        foreach (IEmojiCry observer in observers)
        {
